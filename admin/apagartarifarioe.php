<?php
    ...
    function apagartarifario($data,$et,$mysqli)
    {
      //Sequencia de querys para apagar tarifario
      $sqlselect="SELECT * FROM ELECTRICIDADE WHERE ID_TARIFARIO=" . $et . " AND DATA=" . $data;
      $result=mysql_query($sqlselect) or die(mysql_error());
      $row = mysql_fetch_array($result);
      $tarifasimples=$row['ID_TARIFA_SIMPLES'];
      $tarifabi=$row['ID_TARIFA_BI'];
      $tarifatri=$row['ID_TARIFA_TRI'];
      $mysqli->autocommit(FALSE);
      if($tarifasimples!=NULL)
        $mysqli->query("DELETE FROM TARIFA_SIMPLES WHERE IDTARIFA_SIMPLES=" . $tarifasimples);
      if($tarifabi!=NULL)
       $mysqli->query("DELETE FROM TARIFA_BI WHERE IDTARIFA_BI=" . $tarifabi);
      if($tarifatri!=NULL)
       $mysqli->query("DELETE FROM TARIFA_TRI WHERE IDTARIFA_TRI=" . $tarifatri);
       $mysqli->query("DELETE FROM ELECTRICIDADE WHERE ID_TARIFARIO=" . $et . " AND DATA=" . $data);
       if($mysqli->commit())
        {
            $mysqli->close();
            return true;
...
?>    
  ...
  <div class="container">
    <?php
    if(isset($data) && isset($et))
    {
    ?>
      <h3 align="center">Apagar Tarifário:</h3>
      <hr>
  ...
  <div name="confirmeapagar" id="confirmeapagar" class="container marketing">  
    <fieldset align="center" >
        <table width='0%' height='0%' align="center" border="0" bordercolor="#000000" style="background-color:#FFFFFF" >
          ...
          <label>
          <?php
                $sucesso=apagartarifario($data,$et,$mysqli);
          if($sucesso==TRUE)
          {
            ?>
            <p> Tarifário Apagado</p>
            <?php
          }
          else
          {
            ?>
            <p> Erro ao Apagar Tarifário</p>
            <?php
